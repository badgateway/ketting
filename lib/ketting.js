...
Ketting.prototype = {
  ...
  fetch : function(input, init) {
    ...
    var newInit = this.fetchInit;
    var headerName;

    if (init) {
      Object.assign(newInit, this.fetchInit, init);

      // Special casing headers. It might be an object.
      // Initializing it with the headers from 'init'.
      newInit.headers = new fetch.Headers(this.init.headers);

      // Now setting any headers from fetchInit, if they didn't exist
      // already.
      for(headerName in this.fetchInit.headers) {
        if (!newInit.headers.has(headerName)) {
          newInit.headers.set(headerName, this.fetchInit.headers[headerName]);
        }
      }
      ...
      // Special casing headers. We just want to merge them in, but only if they
      // weren't already set.
      if (input.headers) {
        delete newInit.headers;
        for(headerName in this.fetchInit.headers) {
          if (!input.headers.has(headerName)) {
            input.headers.set(headerName, this.fetchInit.headers[headerName]);
          }
        }
      }
